...
struct Squiggle: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let pathRect = path.boundingRect
            path = path.offsetBy(dx: rect.midX - pathRect.midX, dy: rect.midY - pathRect.midY)
            let scaleX: CGFloat = rect.width / pathRect.height // Note the flipped aspect ratio here
            let scaleY: CGFloat = rect.height / pathRect.width // Flipping width and height for vertical orientation
            let scale = min(scaleX, scaleY)
            
            // Apply rotation and scale
            var transform = CGAffineTransform(rotationAngle: .pi / 2) // Rotate 90 degrees
            transform = transform.scaledBy(x: scale, y: scale)
            path = path.applying(transform)
            
            // Center the path in the rect
            let rotatedRect = path.boundingRect
            path = path.offsetBy(dx: rect.midX - rotatedRect.midX, dy: rect.midY - rotatedRect.midY)
